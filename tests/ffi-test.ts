 ...
 *   4. linkSymbols() — compile inline C source (NEEDS TinyCC — new!)
 */

const { dlopen, callback, linkSymbols, readCString } = require("bun:ffi");
...
function test(name, fn) {
  try {
    ...
    if (result instanceof Promise) {
      ...
      throw new Error("async tests not supported");
    }
    passed++;
    console.log(`  ✅ ${name}`);
  } catch (e) {
    if (e.message && e.message.includes("not available in this build")) {
      skipped++;
...
test("load libc.so and call strlen", () => {
  const lib = dlopen("/system/lib64/libc.so", {
    strlen: { args: ["cstring"], returns: "usize" },
  });
  const len = lib.symbols.strlen("hello world");
  assertEqual(len, 11, "strlen('hello world')");
  console.log(`     → strlen("hello world") = ${len}`);
});

test("load libc.so and call strcmp", () => {
  const lib = dlopen("/system/lib64/libc.so", {
    strcmp: { args: ["cstring", "cstring"], returns: "i32" },
  });
  const eq = lib.symbols.strcmp("abc", "abc");
  const lt = lib.symbols.strcmp("abc", "abd");
...
test("readCString from libc strdup", () => {
  ...
  const dup = lib.symbols.strdup("hello ffi");
...
test("callback with string arg", () => {
  const cb = callback({ returns: "usize", args: ["cstring"] }, (s) => {
    return s.length;
  });
  const result = cb("hello");
  assertEqual(result, 5, "callback('hello') should return 5");
  console.log(`     → callback('hello') = ${result}`);
});
...
console.log("");
console.log("🔗 Test 4: linkSymbols() — compile inline C (needs TinyCC)");

test("linkSymbols with inline C source", () => {
