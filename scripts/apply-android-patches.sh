...
# =====================================================================
# PATCH 11: src/runtime/ffi/FFI.h — untag Android TBI pointers in FFI
# =====================================================================
# ROOT CAUSE: Android 11+ on aarch64 uses TBI (Top Byte Ignore) for MTE.
# malloc() returns pointers with a non-zero top byte (the tag), e.g.
# 0x3c0000700000. When Bun's FFI passes these tagged pointers to native
# code, Bionic's MTE check detects the tag was truncated in syscalls and
# aborts with "Pointer tag for 0x... was truncated" → SIGABRT/SIGBUS.
#
# FIX: Strip the top byte (bits 56-63) from pointers at the JS↔native
# boundary. All FFI pointer marshalling goes through 3 macros in FFI.h:
#   JSVALUE_TO_PTR()                 — JS→Native pointer extraction
#   JSVALUE_TO_TYPED_ARRAY_VECTOR()  — JS→Native ArrayBuffer backing store
#   PTR_TO_JSVALUE()                 — Native→JS pointer encoding (double)
#
# We add FFI_UNTAG_POINTER() and apply it in all 3. The mask
# 0x00FFFFFFFFFFFFFF clears the top byte while preserving the 48-bit
# address. Untagged pointers bypass MTE checks.
FFI_H="src/runtime/ffi/FFI.h"
if [ -f "$FFI_H" ]; then
    if grep -q "$PATCH_MARKER" "$FFI_H" 2>/dev/null; then
        echo "  [SKIP] $FFI_H already patched"
    else
        echo "  [PATCH] $FFI_H (untag TBI pointers for Android)"
        python3 <<'PYEOF'
import sys
...
# 1. Add the FFI_UNTAG_POINTER macro after the BUN_FFI_IMPORT define
anchor1 = '#define BUN_FFI_IMPORT\n#endif'
if anchor1 not in content:
    print("    [FAIL] could not find BUN_FFI_IMPORT anchor")
    sys.exit(1)

untag_macro = '''#define BUN_FFI_IMPORT
#endif

// ANDROID_TERMUX_FIX: Strip TBI (Top Byte Ignore) tag from pointers on
// Android. Android 11+ tags heap pointers with the top byte for MTE.
// When Bun's FFI passes tagged pointers to native code, Bionic detects
// the tag was truncated in syscalls and aborts with:
//   "Pointer tag for 0x... was truncated"
// Untagged pointers (top byte = 0) bypass MTE checks.
// Mask: 0x00FFFFFFFFFFFFFF clears bits 56-63, preserves 48-bit address.
#if defined(__ANDROID__) || defined(__BIONIC__)
#define FFI_UNTAG_POINTER(p) ((void*)((uintptr_t)(p) & 0x00FFFFFFFFFFFFFFULL))
#else
#define FFI_UNTAG_POINTER(p) (p)
#endif'''

content = content.replace(anchor1, untag_macro, 1)

# 2. Fix JSVALUE_TO_TYPED_ARRAY_VECTOR — untag the backing store pointer
old_tav = '''static void* JSVALUE_TO_TYPED_ARRAY_VECTOR(EncodedJSValue val) {
  return *(void**)((char*)val.asPtr + JSArrayBufferView__offsetOfVector);
}'''
new_tav = '''static void* JSVALUE_TO_TYPED_ARRAY_VECTOR(EncodedJSValue val) {
  void* ptr = *(void**)((char*)val.asPtr + JSArrayBufferView__offsetOfVector);
  return FFI_UNTAG_POINTER(ptr);
}'''
if old_tav not in content:
    print("    [FAIL] could not find JSVALUE_TO_TYPED_ARRAY_VECTOR")
    sys.exit(1)
content = content.replace(old_tav, new_tav, 1)

# 3. Fix JSVALUE_TO_PTR — untag the returned pointer
old_jtp = '''  // Assume the JSValue is a double
  val.asInt64 -= DoubleEncodeOffset;
  return (void*)(uintptr_t)val.asDouble;
}'''
new_jtp = '''  // Assume the JSValue is a double
  val.asInt64 -= DoubleEncodeOffset;
  return FFI_UNTAG_POINTER((void*)(uintptr_t)val.asDouble);
}'''
if old_jtp not in content:
    print("    [FAIL] could not find JSVALUE_TO_PTR double path")
    sys.exit(1)
content = content.replace(old_jtp, new_jtp, 1)

# Also untag the int32 path in JSVALUE_TO_PTR
old_int32 = '''  if (JSVALUE_IS_INT32(val)) {
    return (void*)(uintptr_t)JSVALUE_TO_INT32(val);
  }'''
new_int32 = '''  if (JSVALUE_IS_INT32(val)) {
    return FFI_UNTAG_POINTER((void*)(uintptr_t)JSVALUE_TO_INT32(val));
  }'''
if old_int32 in content:
    content = content.replace(old_int32, new_int32, 1)

# Also untag the typed array path in JSVALUE_TO_PTR
old_ta = '''  if (JSCELL_IS_TYPED_ARRAY(val)) {
    return JSVALUE_TO_TYPED_ARRAY_VECTOR(val);
  }'''
new_ta = '''  if (JSCELL_IS_TYPED_ARRAY(val)) {
    return FFI_UNTAG_POINTER(JSVALUE_TO_TYPED_ARRAY_VECTOR(val));
  }'''
if old_ta in content:
    content = content.replace(old_ta, new_ta, 1)

# 4. Fix PTR_TO_JSVALUE — untag before double encoding (prevents precision loss)
old_ptj = '''static EncodedJSValue PTR_TO_JSVALUE(void* ptr) {
  EncodedJSValue val;
  if (ptr == 0) {
  ...
  val.asInt64 += DoubleEncodeOffset;
  return val;'''
new_ptj = '''static EncodedJSValue PTR_TO_JSVALUE(void* ptr) {
  EncodedJSValue val;
  ptr = FFI_UNTAG_POINTER(ptr);
  if (ptr == 0) {
    val.asInt64 = TagValueNull;
    return val;
  }
  ...
  val.asInt64 += DoubleEncodeOffset;
  return val;'''
if old_ptj not in content:
    print("    [FAIL] could not find PTR_TO_JSVALUE")
    sys.exit(1)
content = content.replace(old_ptj, new_ptj, 1)

with open("src/runtime/ffi/FFI.h", "w") as f:
    f.write(content)

print("    [OK] Added FFI_UNTAG_POINTER to JSVALUE_TO_PTR, JSVALUE_TO_TYPED_ARRAY_VECTOR, PTR_TO_JSVALUE")
PYEOF
        verify_patch "$FFI_H" "$PATCH_MARKER" || true
    fi
fi

# =====================================================================
# PATCH 12: src/runtime/ffi/FFIObject.zig — untag TBI pointers on Zig side
# =====================================================================
# The Zig-side ptr() and getPtrSlice() functions also handle pointers.
# ptr() reads array_buffer.ptr (tagged by malloc) and stores it in a
# JSValue. getPtrSlice() takes a pointer from JS and creates a slice.
# Both need untagging on Android to prevent Bionic MTE crashes.
FFI_OBJ="src/runtime/ffi/FFIObject.zig"
if [ -f "$FFI_OBJ" ]; then
    if grep -q "$PATCH_MARKER" "$FFI_OBJ" 2>/dev/null; then
        echo "  [SKIP] $FFI_OBJ already patched"
    else
        echo "  [PATCH] $FFI_OBJ (untag TBI pointers for Android)"
        python3 <<'PYEOF'
import sys

with open("src/runtime/ffi/FFIObject.zig", "r") as f:
    content = f.read()

# Add an untag helper function after the imports
# Find the first pub fn in the file to insert before it
anchor = "const ValueOrError = union(enum) {"
if anchor not in content:
    print("    [FAIL] could not find ValueOrError anchor")
    sys.exit(1)

helper = '''// ANDROID_TERMUX_FIX: Strip TBI (Top Byte Ignore) tag from pointers.
// Android 11+ tags heap pointers with the top byte for MTE. When we
// pass tagged pointers to native code or Bionic syscalls, they crash
// with "Pointer tag was truncated". This helper strips the top byte.
inline fn ffiUntagPointer(addr: usize) usize {
...
# Fix ptr_() — untag the address from array_buffer.ptr
old_ptr = "    var addr: usize = @intFromPtr(array_buffer.ptr);"
new_ptr = "    var addr: usize = ffiUntagPointer(@intFromPtr(array_buffer.ptr));"
if old_ptr in content:
    content = content.replace(old_ptr, new_ptr, 1)
    print("    [OK] Patched ptr_() to untag array_buffer.ptr")
else:
    print("    [WARN] ptr_() anchor not found (may have changed)")

# Fix getPtrSlice() — untag the address from JS value
old_slice = "    var addr = @as(usize, @bitCast(num));"
new_slice = "    var addr = ffiUntagPointer(@as(usize, @bitCast(num)));"
if old_slice in content:
    content = content.replace(old_slice, new_slice, 1)
    print("    [OK] Patched getPtrSlice() to untag pointer from JS")
else:
    print("    [WARN] getPtrSlice() anchor not found (may have changed)")

# Fix ptrWithoutTypeChecks — untag the pointer from array.ptr()
old_ptc = "    return JSValue.fromPtrAddress(@intFromPtr(array.ptr()));"
new_ptc = "    return JSValue.fromPtrAddress(ffiUntagPointer(@intFromPtr(array.ptr())));"
if old_ptc in content:
    content = content.replace(old_ptc, new_ptc, 1)
    print("    [OK] Patched ptrWithoutTypeChecks() to untag")
else:
    print("    [WARN] ptrWithoutTypeChecks() anchor not found")

with open("src/runtime/ffi/FFIObject.zig", "w") as f:
    f.write(content)
PYEOF
        verify_patch "$FFI_OBJ" "$PATCH_MARKER" || true
    fi
fi
